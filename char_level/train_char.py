...
class Arguments():
    def __init__(self):
        ...
        self.no_cuda = False
        self.mode = 'C3'
        self.path = '/scratch/yw3918/RNN_model/C2/C2_1.pt'
...
def train_test(char_encoder, model, dset_loaders, epoch, phase, optimizer, args, logger, use_gpu, save_path):
    ...
    results = []
    for batch_idx, (train_data, seq_tensors, _) in enumerate(dset_loaders[phase]):
        ...
        if phase == 'train':
            ...
            with torch.no_grad():
                ...
                loss = criterion(train_outputs, train_targets, mu, sig)
                optimizer.zero_grad()
                #results.append([train_inputs.detach(), train_outputs.detach()])
        # stastics
